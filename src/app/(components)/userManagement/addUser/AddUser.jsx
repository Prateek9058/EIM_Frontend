...
import { Typography, Grid, TextField, Divider, IconButton, Box, FormControl, MenuItem, Select, InputLabel, FormHelperText } from '@mui/material';
...
export default function AddUser({ open, setOpen }) {
    ...
    const AddRole = async () => {
        try {
            const response = await axiosInstance.get(`/role/getRolesWithLevel?level=${level}`)
    ...
    const AddManager = async () => {
        try {
            const response = await axiosInstance.get(`/user/getAllParents?level=${level}`)
    ...
    return (
        <React.Fragment>
            <ToastComponent />
            {open1 && <AddRole1 open={open1} setOpen={setOpen1} />}
            <Dialog
                open={open}
                maxWidth={"sm"}
                onClose={handleClose}
                ...
                <form onSubmit={handleSubmit(upLoadFile)} noValidate>
                    ...
                    <DialogContent >
                        <Grid container rowGap={2}>
                            <Grid container justifyContent={"space-between"} >
                                ...
                                <Button variant='outlined' endIcon={<AddIcon />} onClick={handleOpen}>Add New Role</Button>
                            </Grid>
                            {file ? <Grid container direction={"column"} sx={{ marginBottom: "16px" }}>
                                ...
                                alignItems={"center"}
                                sx={{
                                    border: `3px dotted #fff`, padding: 2, borderRadius: "8px"
                                }} {...getRootProps()}>
                                <IconButton size='large'><CloudUploadOutlinedIcon fontSize='large' /></IconButton>
                                <Typography variant='h5'>Select a logo file or drag and drop here</Typography>
                                ...
                                <Grid item xs={6}>
                                    <Grid container rowGap={3}>
                                        <TextField
                                            ...
                                            value={level}
                                            {...register("level", {
                                                required: "level is required!", min: { value: 1, message: "Level cannot be negative!" }
                                                , max: { value: 6, message: "level cannot be greater than 6 " },
                                    ...
                                    <Grid container rowGap={3}>
                                        <FormControl fullWidth error={!!errors.role}>
                                            ...
                                            <Select
                                                ...
                                                value={selectRole}
                                                label="Select Role"
                                                {...register("role", { required: "Role is required!", onChange: (e) => { handleRole(e) } })}
                                            >
                                        ...
                                        <FormControl fullWidth error={!!errors.parent}>
                                            ...
                                            <Select
                                                ...
                                                {...register("parent", { required: "Manager is required!", onChange: (e) => { handleManager(e) } })}
                                            >
                                            ...
                                            <FormHelperText> {errors?.parent && errors?.parent?.message}</FormHelperText>
                                        </FormControl>
                                    ...
                                    <Grid container rowGap={3}>
                                        ...
                                        {<FormControl fullWidth error={!!errors.subAdmin}>
                                            ...
                                            <Select
                                                ...
                                                label="subAdmin"
                                                {...register("subAdmin", { required: "Role is required!", onChange: (e) => { handleSubAdmin(e) } })}
                                            >
                                                {subAdmin && subAdmin?.map((item, index) => (
                                                    <MenuItem key={index} value={item?._id} >{item?.userName}</MenuItem>
                                                ))}

                                            </Select>
                                            <FormHelperText> {errors.subAdmin && errors.subAdmin.message}</FormHelperText>
                                        </FormControl>
                                        }
                                    </Grid>
                                </Grid>
                    ...
                    <DialogActions sx={{ justifyContent: 'center', }} >
                        <Button size='large' variant='contained' type='submit'>Add User</Button>
                    </DialogActions>
                </form>
            </Dialog>
        </React.Fragment >
