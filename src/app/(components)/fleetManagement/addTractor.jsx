...
export default function AddUser({ open, setOpen }) {
    ...
    const onsubmit = async() => {
        ...
        try {
            const response=await axiosInstanceImg.post('/fleet/bulkUpload', formData,
                {
            ...
            ><ToastComponent/>
                <form onSubmit={handleSubmit(submitDetails)}  >
                            ...
                            <Grid container spacing={2} >
                                <Grid item xs={6} >
                                    <Grid container rowGap={3} >
                                        ...
                                        <TextField
                                            ...
                                            {...register("capacity", { required: "licence is required!" })}
                                ...
                                <Grid item xs={6}>
                                    ...
                                    <FormControl fullWidth error={!!errors.batteryId}>
                                            <InputLabel id="demo-simple-select-label">Select Battery </InputLabel>
                                            <Select
                                                labelId="battery"
                                                id="battery"
                                                value={batteryId}
                                                label="battery"
                                                {...register("battery", { required: "Battery  is required!", onChange: (e) => { handleBattery(e) } })}
                                            >
                                                {battery && battery?.map((item, index) => (
                                                    <MenuItem key={index} value={item?._id} >{item?.batteryId}</MenuItem>
                                                ))}

                                            </Select>
                                            <FormHelperText>{errors.battery && errors.battery.message}</FormHelperText>
                                        </FormControl>
                                        <TextField
                                            fullWidth
                    ...
                    <DialogActions sx={{ justifyContent: "center" }}>
                        {file?<Button size='large' variant='contained' onClick={onsubmit}>Add E-Tractor</Button>:<Button size='large' variant='contained' type='submit'>Add E-Tractor</Button>}
                    </DialogActions>
                </form>
            </Dialog>
        </React.Fragment>
